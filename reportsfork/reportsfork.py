...
_ = Translator("ReportsFork", __file__)

# XXX ya ya hardcoding a role name
STAFF_ROLE_NAME = "staff"

log = logging.getLogger("red.reportsfork")


@cog_i18n(_)
class ReportsFork(commands.Cog):
    ...
    async def send_report(self, ctx: commands.Context, msg: discord.Message, guild: discord.Guild):
        ...
        if await self.bot.embed_requested(channel):
            ...
            if staff_role is None:
                ...
                send_content = _("{staff_role.mention}").format(staff_role=staff_role)
        else:
            em = None
            ...
            if staff_role is not None:
                send_content = _("{staff_role.mention}: ").format(staff_role=staff_role) + send_content
            send_content += "\n" + report
